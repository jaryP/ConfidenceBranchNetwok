...
def my_app() -> None:
    ...
    for path in paths:
        for experiment_path in [f for f in os.listdir(path)
            ...
            if os.path.exists(os.path.join(experiment_path, 'results.json')):
                # log.info('Model loaded')
                with open(os.path.join(experiment_path, 'results.json'), 'r') \
                        as json_file:
                    results = json.load(json_file)

                plot_dirs = os.path.join(experiment_path, 'plots')
                ...
                if 'entropy_results' in results:
                    cumulative_scores_fig = plot_scores(results['entropy_results'])
                    cumulative_scores_fig.savefig(os.path.join(
